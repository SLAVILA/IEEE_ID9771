...
@b_longo_prazo.route("/_longo_prazo_obter", methods=["POST"])
def _longo_prazo_obter():
    tipo = request.form.get('tipo')
    m = request.form.get('m')
    nome_arquivo = f'longo_prazo_saida_{tipo}.json'
    nome_arquivo_salto = f'longo_prazo_saida_{tipo}_salto.json'
    output_file = os.path.join('web', 'static', 'tasks_saida', 'longo_prazo', m, 'output', nome_arquivo)
    output_file_salto = os.path.join('web', 'static', 'tasks_saida', 'longo_prazo', m, 'output', nome_arquivo_salto)
...
def longo_prazo():
    ...
    for yy in range(4):
        ...
        VAR05 = np.zeros((len(datesIbc)-LEN,lenP,2))
        CVAR05 = np.zeros((len(datesIbc)-LEN,lenP,2))

        MR_VAR05 = np.zeros((len(datesIbc)-LEN,lenP,2))
        MR_CVAR05 = np.zeros((len(datesIbc)-LEN,lenP,2))

        L_MRJ = np.zeros((len(datesIbc)-LEN, lenP+1))
        M_MRJ = np.zeros((len(datesIbc)-LEN, lenP+1))
        U_MRJ = np.zeros((len(datesIbc)-LEN, lenP+1))

        L_MR = np.zeros((len(datesIbc)-LEN, lenP+1))
        M_MR = np.zeros((len(datesIbc)-LEN, lenP+1))
        U_MR = np.zeros((len(datesIbc)-LEN, lenP+1))


        for ll in np.arange(lenP,len(datesIbc)-LEN):
...
# Gráfico da análise de risco para cada data
    ...
    for yy in range(4): #Alteração_1
        ...
        for jj in np.arange(lenP,len(datesIbc)-LEN):
