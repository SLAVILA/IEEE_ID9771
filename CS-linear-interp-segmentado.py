...
def get_GPR(path):
    df_temp = pd.read_csv(path, parse_dates=['date'], dayfirst=True).rename(columns={"date" : "data"})
    df_temp = df_temp.set_index('data')
    return df_temp
...
def get_restricoes(paths):
    for path in paths:
        df_temp = pd.read_csv(path, delimiter=';')#.astype({'data' : 'datetime64[ns]'})#.reset_index()
        df_temp.data = pd.to_datetime(df_temp.data, format='%Y')
        print(df_temp.columns)
        df_temp = get_var(df_temp)
    return df_temp
...
df_linear_intep = get_base(paths_linear_interp)

path_restricoes = data_path("restricoes_trans")
df_restricoes = get_restricoes(path_restricoes)
...
df_lin_interp = pd.concat(df_linear_intep, axis=1)

df_geral = pd.concat([pib, ibc, oni, cmo_s, cmo_se, cmo_n, cmo_ne, proj_c_M0, proj_c_M1, proj_c_M2, proj_c_M3, proj_c_M4, gpr, earm_SIN, earm_N, earm_NE, earm_S, earm_SE, df_restricoes, tna], axis=1)
...
#maturacoes = [ "VWAP_M4", "VWAP_M3", "VWAP_M2", "VWAP_M1", "VWAP_M0"]
#features = ["ENA_hist", "clim", "ENA_hist-clim", "VWAP_M4", "VWAP_M3", "VWAP_M2", "VWAP_M1", "VWAP_M0"]
...
def get_correlation(df):
    correlacao = {}
    for maturacao in df.columns:
        correlacao[maturacao] = []
        for feature in df.columns:
            if maturacao == feature:
                correlacao[maturacao].append(None)
                continue
            correlacao[maturacao].append(scipy.stats.spearmanr(df[maturacao], df[feature]).correlation)   # Spearman's rho
    correlacao = pd.DataFrame(correlacao, index=df.columns)
    return correlacao
...
correlac = rolling(df, lambda a: get_correlation(a), pd.Timedelta("365D"))
#sns.heatmap(data=correlac, annot=True)

# +

import matplotlib.ticker as ticker
x = {col: [] for col in correlac.iloc[0]}

for d in correlac.index:
    for n_name in range(len(names)):
        x[names[n_name]].append(correlac[d][names[n_name]])

for n_name in names:
    CS_temp = pd.DataFrame(x[n_name], index=correlac.index)
    CS_temp_var = CS_temp[df_lin_interp.columns]
    fig, ax = plt.subplots(figsize=(30,20))
    data = CS_temp_var.T
    sns.set(font_scale=1.4)
    sns.heatmap(data, annot=False, cmap="PiYG", yticklabels=1, ax=ax)#, xticklabels = data.columns.year.unique())
    ...
    if not n_name.startswith("linear_interp/"):
        ax.set_title(n_name)
        name = "CS-linear-interp/var/"+n_name
    else:
        ax.set_title(n_name[13:])
        name = "CS-linear-interp/var/"+n_name[13:]
...
for n_name in names:
    ...
    sns.heatmap(data=CS_temp_ena.T, annot=False, cmap="PiYG", yticklabels=1)
    ax.set(xticks=([0, 53, 104, 157, 212, 280]))
    ax.set(xticklabels = (['2019-01', '2019-07', '2020-01', '2020-07','2021-01','2021-07']))
    ax.set_xticklabels(ax.get_xticklabels(), rotation=0, horizontalalignment='left')
    ...
    if not n_name.startswith("linear_interp/"):
        ax.set_title(n_name)
        name = "CS-linear-interp/ena/"+n_name
    else:
        ax.set_title(n_name[13:])
        name = "CS-linear-interp/ena/"+n_name[13:]
    plt.savefig(name, dpi=400, bbox_inches="tight")
...
for n_name in names:
    ...
    fig, ax = plt.subplots(figsize=(30,20))
    sns.heatmap(data=CS_temp_geral.T, annot=False, cmap="PiYG", yticklabels=1)
    ax.set(xticks=([0, 53, 104, 157, 212, 280]))
    ax.set(xticklabels = (['2019-01', '2019-07', '2020-01', '2020-07','2021-01','2021-07']))
    ax.set_xticklabels(ax.get_xticklabels(), rotation=0, horizontalalignment='left')
    if not n_name.startswith("linear_interp/"):
        ax.set_title(n_name)
        name = "CS-linear-interp/geral/"+n_name
    else:
        ax.set_title(n_name[13:])
        name = "CS-linear-interp/geral/"+n_name[13:]
    plt.savefig(name, dpi=400, bbox_inches="tight")
